...
def _build_with_docker(
    ...
    include_dirs = [Path(item) for item in build_spec.get("include_dirs", [])]
    cflags = list(build_spec.get("cflags", []))
    source_rel = source_path_value.relative_to(config.state_dir).as_posix()
    include_rel = [item.relative_to(PROJECT_ROOT).as_posix() for item in include_dirs]
    ...
    for arch, output_path in output_paths.items():
        ...
        command = [
            ...
            "-v",
            f"{config.state_dir.resolve()}:/state",
            "-v",
