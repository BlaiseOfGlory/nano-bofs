...
void netuserinfo(wchar_t *username, wchar_t *hostname)
{
    ...
    DWORD dwGroupRead = 0;
    DWORD dwGroupTotal = 0;
    DWORD dwLocalGroupRead = 0;
    DWORD dwLocalGroupTotal = 0;
    DWORD lastset = 0;

    umStatus = NETAPI32$NetUserModalsGet(hostname, 0, (LPBYTE *)&pUserModals);
    ...
    if (nStatus == NERR_Success)
    {
        ...
        internal_printf((pBuf4->usri4_flags & UF_ACCOUNTDISABLE) ? "No\n" : "Yes\n");

        internal_printf("Trusted for delegation:\t\t");
        internal_printf((pBuf4->usri4_flags & UF_TRUSTED_FOR_DELEGATION) ? "Yes\n" : "No\n");

        internal_printf("Dont require preauth:\t\t");
        internal_printf((pBuf4->usri4_flags & UF_DONT_REQUIRE_PREAUTH) ? "Yes\n" : "No\n");
        ...
        internal_printf("\nPassword required:\t\t");
        internal_printf((pBuf4->usri4_flags & UF_PASSWD_NOTREQD) ? "No\n" : "Yes\n");
        internal_printf("User may change password:\t\t");
        internal_printf((pBuf4->usri4_flags & UF_PASSWD_CANT_CHANGE) ? "No\n\n" : "Yes\n\n");
        ...
        lgStatus = NETAPI32$NetUserGetLocalGroups(
            NULL,
            username,
            0,
            0,
        ...
        gStatus = NETAPI32$NetUserGetGroups(
            hostname,
            username,
            0,
            (LPBYTE *)&pGroupInfo,
...
)
{
    ...
    wchar_t username_buffer[sizeof(NANO_USERNAME) / sizeof(NANO_USERNAME[0])];
    ...
    memcpy(username_buffer, NANO_USERNAME, sizeof(NANO_USERNAME));
    memcpy(domain_buffer, NANO_DOMAIN, sizeof(NANO_DOMAIN));
    domain = *domain_buffer == 0 ? NULL : domain_buffer;

    netuserinfo(username_buffer, domain);
    printoutput(TRUE);
}
