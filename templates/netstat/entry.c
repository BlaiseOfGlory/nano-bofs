...
void ShowTcp6Table()
{
    ...
    dwSize = 0;
    error = IPHLPAPI$GetExtendedTcpTable(NULL, &dwSize, TRUE, AF_INET6, TCP_TABLE_OWNER_PID_ALL, 0);
    if (error != ERROR_INSUFFICIENT_BUFFER)
    {
        return;
    }
    ...
    error = IPHLPAPI$GetExtendedTcpTable(ptTable, &dwSize, TRUE, AF_INET6, TCP_TABLE_OWNER_PID_ALL, 0);
    if (error)
    {
...
VOID go(IN PCHAR Buffer, IN ULONG Length)
{
    ...
    static const int NANO_CHOICES = __NANO_CHOICE_MASK__;
    int user_choices = NANO_CHOICES;
