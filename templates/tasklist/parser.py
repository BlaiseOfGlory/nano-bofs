...
NAME = "tasklist"
DESCRIPTION = "List running processes on the local or specified computer."
VARIABLES = [
    {
        "name": "server",
        "required": False,
...
def build_resource(server: str) -> str:
    if server == "":
        return r"root\cimv2"
    return rf"\\{server}\root\cimv2"
...
def build_plan(args: argparse.Namespace) -> dict[str, object]:
    ...
    resource = build_resource(server)
    return {
        "template_path": DEFAULT_TEMPLATE,
        "source_name": f"{NAME}.c",
        "artifact_basename": NAME,
        "placeholders": {
            "__NANO_RESOURCE__": resource,
        },
        "metadata": {
            "final_server": server,
...
def main() -> None:
    ...
    try:
        ...
        print(f"final_resource={plan['metadata']['final_resource']}")
