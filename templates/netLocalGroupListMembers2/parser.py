...
NAME = "netLocalGroupListMembers2"
DESCRIPTION = "List local group members in bofhound-compatible output, either for one group or the default interesting groups."
VARIABLES = [
    {
        "name": "groupname",
        "required": False,
        "description": "Optional local group name. Leave empty to query the default interesting local groups.",
        "shape": "Single local group name.",
        "example": "Administrators",
        "validation": [
            "Leave empty to query the default interesting local groups.",
...
DEFAULT_TEMPLATE = Path(__file__).resolve().parents[1] / "netlocalgroup2" / "entry.c"
COMMON_INCLUDE_DIR = (
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        nargs="?",
        default="",
        help="Optional local group name. Leave empty to query the default interesting local groups.",
    )
    parser.add_argument(
        "server",
        nargs="?",
...
def build_plan(args: argparse.Namespace) -> dict[str, object]:
    ...
    return {
        ...
        "metadata": {
            ...
            "final_server": server,
            "queries_interesting_groups": groupname == "",
...
def main() -> None:
    ...
    try:
        ...
        print(f"queries_interesting_groups={plan['metadata']['queries_interesting_groups']}")
