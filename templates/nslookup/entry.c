...
)
{
    ...
    static const char NANO_LOOKUP[] = "__NANO_LOOKUP__";
    static const char NANO_SERVER[] = "__NANO_SERVER__";
    char lookup_buffer[sizeof(NANO_LOOKUP)];
    char server_buffer[sizeof(NANO_SERVER)];
    char *server = NULL;
    unsigned short type = (unsigned short)__NANO_RECORD_TYPE__;
    ...
    memcpy(lookup_buffer, NANO_LOOKUP, sizeof(NANO_LOOKUP));
    memcpy(server_buffer, NANO_SERVER, sizeof(NANO_SERVER));
    server = *server_buffer == 0 ? NULL : server_buffer;

    query_domain(lookup_buffer, type, server);
    printoutput(TRUE);
}
#endif
